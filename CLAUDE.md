...
- **Microskills** — single-purpose, black-box units with a declared input contract and output schema. Internals may branch, loop, classify, use tools — even pause for a human (declare `AskUserQuestion` in `runtime.allowed_tools` and the compiler runs the node at an orchestrator checkpoint). `validate-microskill` checks only structure (sections, name, inputs table, output schema) and warns past ≤10 steps — it has no control-flow opinion; the "is this one job?" test lives in the create pipeline's semantic review, not a regex. A unit graduates to a workflow when it must compose *other* components — call another microskill/agent/workflow, or orchestrate a multi-node graph (fan-out, gated loops).

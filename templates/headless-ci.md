...
| `microskill-create` | Yes | `--profile autonomous` (declares `gate_mode: auto` + `default: approve` on the plan gate) |
| `workflow-create` | Yes | `--profile autonomous` (same mechanism) |
| `build-workflow-from-plan` | Yes | gate-free; runs unattended given a plan |

...
- Reuse is profile-centric. A `use:` node reuses a microskill; the variable part is which profile fits — `base`, an existing overlay, or a NEW profile the implementer must mint. A `workflow:` node invokes another workflow: a STATIC one (explicit inputs) is compile-time INLINED into the host DAG, a `for_each` one stays a runtime nested-workflow checkpoint. Reuse a whole workflow by a `workflow:` node where it composes, or flag `scope_advisory: adapt` when the work is really a variant of an existing workflow.

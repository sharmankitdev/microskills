...
`validate-workflow` checks: schema conformance; unique node ids; every `${id.output}` ref names a real node (a ref implies its dependency edge — a `depends_on` entry restating one is a `warn`); every `${workflow.inputs.x}` is declared in top-level `inputs`; typed field refs against each producer's effective schema (inline, else the resolved microskill schema); cycles; gate/loop/output anchoring; `for_each` requires `as`; `as` is a safe identifier; no `for_each` node inside `loop.body`. `compile-workflow` additionally resolves every `use:` node's profile, classifies delegation, and partitions segments — catching missing profiles and segmentation defects the schema alone cannot.
...
- **Output-schema coherence** — every `${<id>.output.<field>}` referenced anywhere (`inputs`/`prompt`/`when`/`for_each`/`carry`) has a matching property in `<id>`'s **effective** `output_schema` (its inline one when declared, else — for a `use:` node — the microskill's resolved schema). Missing → `block`.
- **Microskill output contract** — a `use:` node should normally declare NO `output_schema` (it inherits the resolved one). When one IS declared: run `resolve-microskill <name> [--profile <p>]` and read its declared `output_schema`. (1) If the microskill declares an `output_schema`, an inline node schema that merely restates it is a `warn` (omit it), and one incompatible with it (different required keys, incompatible types) without a stated narrowing rationale is a `block`. (2) If the microskill declares NO `output_schema` but the node imposes one, that is a `block`: the microskill emits free-form output, so the imposed schema coerces unreliably (it silently produces garbage). Recommend declaring `output_schema` in the microskill's `base.yaml` so its result is structured.
